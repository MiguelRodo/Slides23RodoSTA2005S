...
It contains two files, within the `_raw_data` folder.
- `Advertising.csv`
- `reg_img_exp.png`. This file (and not `Advertising.csv`) must be in the `img` sub-folder of `_raw_data`, i.e. at `_raw_data/img/reg_img_exp.png`.

So, in your working directory you must have these files at these paths:
- `_raw_data/Advertising.csv` (`Advertising.csv`)
- `_raw_data/img/reg_img_exp.png` (`reg_img_exp.png`)
...
- Since we're interested in performing linear regression, we would like to assess upfront the following:
  ...
  - Relationship between response and dependent variables (hopefully linear)
...
#| results: asis
#| echo: false
path_img <- "_raw_data/img/reg_img_exp.png"
