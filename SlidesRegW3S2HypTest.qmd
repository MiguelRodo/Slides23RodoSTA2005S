...
\begin{center}
\includegraphics[width=0.95\textwidth]{_raw_data/img/summary.png}
\end{center}
...
- Suppose that we have the following null hypothesis:
  - $H_0: \ell'\symbf{\beta} = \ell'\symbf{{\beta}}^{(0)}$
    - For example, if $\ell = \begin{pmatrix} 0 & 1 & 0 \end{pmatrix}$ and $\symbf{{\beta}}^{(0)'} = \begin{pmatrix} \beta_0 & 0 & \beta_2 \end{pmatrix}'$ then the null hypothesis is that $\beta_1 = 0$.
  - $H_1: \ell'\symbf{\beta} \neq \ell'\symbf{{\beta}}^{(0)}$
...
\begin{center}
\includegraphics[width=0.66\textwidth]{_raw_data/img/summary-shortened.png}
\end{center}
...
vcov_contrast <- L %*% vcov_beta %*% t(L)

# Compute the F-statistic
F_stat <- t(contrast) %*% solve(vcov_contrast) %*% contrast / nrow(L)
```
...
p_value_F <- if (p_value_F < 0.001) "<0.001" else signif(p_value_F, 3)

# Display the results
c("F-statistic" = F_stat |> signif(3), "p-value" = p_value_F) 
```
