...
- However, the p-value for the effect of $X$ on $Y$ for group "B" is not actually available.
  - For $X$ to have no effect for group "B", we need that $\beta_{X} + \beta_{X:G} = 0$.
...
- *Does the effect of $X$ depend on group $i$?*:
  - This tests whether the effect of $X$ differs between group $i$ and the reference group:
    - $H_0: \beta_{X:G_i} = 0$
    - $H_1: \beta_{X:G_i} \neq 0$
  - We would use a $t$-test to assess the significance of the interaction term.
- *Does the effect of $X$ depend on group?*:
  - This tests whether the effect of $X$ is the same for all groups:
...
- *Does $X$ affect Y for group $i$?*:
  - This tests whether there is a non-zero slope for group $i$:
...
- *Does group $i$ affect $Y$?*:
  - This is a test for whether group $i$ has a different slope or intercept than the reference group:
